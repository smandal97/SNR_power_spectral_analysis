...
def get_radial_profile(dat, cen, nr=20):
    nx = dat.shape[0]
...
def get_radial_avg_img(dat,cen):
    rs,r,f = get_radial_profile(dat,cen)
    #n = dat.shape[0]
...
def get_variance(I,M,r,num=60,sigma_min=1e0,sigma_max=1e2):
...
fits_image_filename = "tycho_snr.fits"     #insert your own fits filename here
with fits.open(fits_image_filename) as hdul:
    data = hdul[0].data

#mask_a and mask_b eliminate the quadrant lines from CCD gaps in Tycho's image
#mask_c makes a circle that cuts out empty corners in the image
...
sigma,V_nomask = get_variance(data, 1.-mask_c, data.shape[0]/2.)
sigma,V_mask   = get_variance(data , mask, data.shape[0]/2.)
sigma,V_mr     = get_variance(np.divide(data, rad_avg, out=np.zeros_like(data), where=rad_avg!=0) , mask, data.shape[0]/2.)
